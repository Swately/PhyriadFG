...
- **S4 — CONVERGENCE (the shipping repo rebuilt IN PLACE, by the six stages, on the LAYERTAB contract)** ·
  **`active`** (host decision 2026-09-03) · boundaries = [`STAGE_CONTRACT.md`](STAGE_CONTRACT.md)
  ...
  - **S4.R7** closing · **TWO HALVES, tracked separately (2026-09-06)**
    - **R7(a)** `--legacy-warp` out of the default = M-R7 · **`done` (2026-09-06)** — the criterion was AMENDED
      first (the old `r ≥ 0.5` clause fails for the shipping default itself; `CONVERGENCE_MASTER_PLAN.md` §R7
      keeps both texts), then the default flipped: **`fg_core.comp` is the product path; `--legacy-warp` is the
      one-token revert and `wap_warp.comp` stays in the tree.** Contract hash unchanged, `--dump-config`
      byte-identical, 47/47 ctest, both paths verified end to end at 7,189 presents each. **MR-8 (the
      operator's eye) has NOT been applied and the default ships ahead of it** — stated in `R7_GATE.md` §8.4
      and in the risk register — **corrected the same
      day:** an earlier version of this row said the `--fg-core` path "needs MOTION_TRUTH T2–T5 built", citing
      `records/BACKLOG_AUDIT.md` A6. T2–T5 have been built and gated since 2026-09-04 (`records/S2_T*_GATE.md`,
      `tools/motion_truth/`) and M1 was already measured for the shipping default
      (`docs/evidence/MOTION_TRUTH_BASELINE.md`); what was missing was the RUN on the other path, not the
      instrument. That run is `records/R7_GATE.md` §7. No proxy closes an M1 gate, and the default itself is
      the operator's.
