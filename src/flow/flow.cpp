// PhyriadFG flow layer. Bodies of the FLOW factories declared in flow/flow.hpp. The flow.cpp-internal
// helpers (nvofa_img / gme_make_pipe / gme_buf_barrier) stay `static`; the main-called factories get
...
#include <algorithm>                // std::max / std::min / std::fill (run_flow body)
#include <vector>
...
// kChangeGateSadZ = the CHANGE GATE threshold. Units = the per-8×8-block SUM |A−B| (the sad_zero/stasis
// scale; ≤8.0 ≈ visually identical). A block whose content did not change (sad_zero < this) cannot be
// dissident no matter what its MV says — the pyramid matcher paints a HALO of object MVs onto flat
// background tiles around/behind every mover, so the gate hugs the mask to the real mover. 0.5 = the
// stasis default scale.
static constexpr float kChangeGateSadZ = 0.5f;
// The DECODE-ONCE scratch. gme_fit_affine decodes the fp16 MV grid (and, when the change gate is
// active, the fp16 sad_zero) ONCE into float scratch via the hal F16C batch (ra::decode_f16), then the
// IRLS + dissidence passes reduce over the floats — byte-identical to a per-block half_to_float (same
// float values → same double accumulation order → identical out6 + dis mask). The scratch is
// F-THREAD-LOCAL and grown ONCE to the grid size (zero-alloc steady state, dogma D-2): gme_fit_affine
// runs only on the F thread (fwd @ have_prev_f, bwd @ do_bwd), so thread_local is the natural
// single-owner home and keeps the function reentrant-clean without passing buffers through the call
// sites. uv = decoded MV interleaved [u,v] per block (mvw·mvh·2 floats); szf = strided sad_zero per
// block (mvw·mvh floats), filled only when the change gate is active (sad_raw != null).
...
double gme_fit_affine(const void* mv_raw, const void* sad_raw, uint32_t mvw, uint32_t mvh,
                             float out6[6], uint8_t* dis_out, bool sub2, int irls_iters){   // default arg (=3) lives on the flow.hpp declaration only
    const int step = sub2 ? 2 : 1;
    const size_t nblk = (size_t)mvw * mvh;
    // ── DECODE-ONCE: mv grid + (gated) sad grid into thread-local float scratch via the hal F16C batch
    // (ra::decode_f16). The IRLS passes + dissidence pass below read these floats — byte-identical to a
    // per-block half_to_float (the hal batch is bit-exact to the scalar decode). Buffers grow ONCE then
    // are reused every pair.
    ...
    // irls_iters defaults to 3 (OLS + 2 reweights); --gme-irls2 passes 2 (OLS + 1 reweight) to shave the
    // marginal iter-2 reweight off the dominant CPU axis.
    ...
    // Dissidence mask + stat over ALL blocks (the mask feeds the matte; the stat is the dis:NN% print).
    ...
    for (uint32_t gy = 0; gy < mvh; ++gy) {
        for (uint32_t gx = 0; gx < mvw; ++gx) {
            ...
            if (dis_out) { int q = (int)(16.0*r + 0.5); if (q > 255) q = 255;
                // THE CHANGE GATE — dissent requires CHANGED content. A halo block's big MV is a lie (its
                // pixels did not change, sad_zero≈0) → byte 0, the mask hugs the real mover. The mover's
                // flat interior also gates out: the cluster becomes the RIM RING, which the object scanline
                // fill + shape-field handle by design. sad_raw layout = RG16F per block (R=sad_best,
                // G=sad_zero); nullable (--no-change-gate). szf is the pre-decoded sad_zero (set iff
                // sad_raw != null).
                if (szf && q > 0) {
...
void nvofa_destroy(VDev& d,NvofaProvider& n){
    ...
    if(n.pool) vkDestroyCommandPool(d.dev,n.pool,nullptr);   // the F-thread-private prep/convert pool
    ...
    if(n.semPrep) vkDestroySemaphore(d.dev,n.semPrep,nullptr); if(n.semOfa) vkDestroySemaphore(d.dev,n.semOfa,nullptr);   // the chain semaphores
    n=NvofaProvider{};
}
...
bool nvofa_alloc_cmds(VDev& d,NvofaProvider& n){
    // CRASH-SAFETY: cmdPrep/cmdConv run on the F-thread; recording them from d.pool (the P-thread's present
    // pool) races P (command pools are externally-synchronized). Give the OFA prep/convert their OWN pool on
    // d.qfam (the graphics+compute family — they submit to A.q2, same family). cmdOfa stays on d.ofaPool (the
    // OFA queue family).
    ...
    // the two binary semaphores that chain prep→OFA→convert on the GPU (1 CPU wait per pair).
...
bool mvsm_create(VDev& d,VkImage mv_img,VkImageView prev_view,const std::vector<uint32_t>& spv,MvSmoothPipe& p){
    ...
    VkPushConstantRange pcr{}; pcr.stageFlags=VK_SHADER_STAGE_COMPUTE_BIT; pcr.size=20; // {alpha,cut,w,h,span}
...
// ── gme-gpu: record the full affine fit for ONE anchor into `cmd`, run ON DEVICE B. ─────────────────
...
bool med_create(VDev& d,VkImageView mv_v,VkImageView mvb_v,VkImageView cur_v,VkImageView out_v,bool with_mvb,const std::vector<uint32_t>& spv,MedianPipe& p){
    ...
    // binding 2 = cur_real (the membership color). NEAREST sampler is fine — the shader samples block
    // CENTRES at exact texel centres ((coord+0.5)/grid maps cleanly), no interpolation.
    ...
    VkPushConstantRange pcr{}; pcr.stageFlags=VK_SHADER_STAGE_COMPUTE_BIT; pcr.size=4; // {sim_thresh}
    VkPipelineLayoutCreateInfo pl{}; pl.sType=VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO; pl.setLayoutCount=1; pl.pSetLayouts=&p.dsl; pl.pushConstantRangeCount=1; pl.pPushConstantRanges=&pcr; vkCreatePipelineLayout(d.dev,&pl,nullptr,&p.layout);
    VkShaderModuleCreateInfo mci{}; mci.sType=VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO; mci.codeSize=spv.size()*sizeof(uint32_t); mci.pCode=spv.data(); VkShaderModule mod; vkCreateShaderModule(d.dev,&mci,nullptr,&mod);
    VkPipelineShaderStageCreateInfo stg{}; stg.sType=VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO; stg.stage=VK_SHADER_STAGE_COMPUTE_BIT; stg.module=mod; stg.pName="main";
    VkComputePipelineCreateInfo cp{}; cp.sType=VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO; cp.stage=stg; cp.layout=p.layout; vkCreateComputePipelines(d.dev,VK_NULL_HANDLE,1,&cp,nullptr,&p.pipe); vkDestroyShaderModule(d.dev,mod,nullptr);
...
// -- Thread F - run_flow. The alias preamble rebinds FgContext's references/array-pointers to the exact
...
void run_flow(FgContext& ctx){
    ...
    auto& lt_fpub_us = ctx.lt_fpub_us;
            // MMCSS token held thread-local → RAII AvRevert at thread exit. Inactive (no AVRT call) for
            // --pin-test modes 0-4 / no --pin (byte-identical when off).
            phyriad::hw::MmcssToken mmcss_f;
            // Pin the FLOW thread to the SPSC producer core AND elevate it to HIGHEST (strictly below the
            // present thread's TIME_CRITICAL) — prevents the F-thread being starved off the scheduler under
            // combined GPU+CPU saturation. Gated on cfg.pin_threads; skip UINT32_MAX for the pin. On failure
            // log + continue (do NOT crash). DEFAULT-OFF -> byte-identical.
            if(cfg.pin_threads){
                // --pin-test selector: affinity in modes 0/1/3; F HIGHEST-elevate in modes 0/2.
                ...
                // --pin-test mode 5 = MMCSS-COMPOSITE: F gets SOFT affinity (ideal-processor hint,
                // migratable so an unpinned elevated F migrates to a freer core) + MMCSS 'Capture'/HIGH
                // (kept below P's 'Pro Audio'/CRITICAL), with elevate_thread_rt(false)=HIGHEST as the
                // fallback when MMCSS is inactive.
                if(cfg.pin_test==5){
            ...
            // The F-thread flow-submit lane. Multi-GPU → the B.q path. single_gpu → A.q2 (NOT A.q, which
            // would collide with present), serialized vs the C-thread convert by a_q2_mtx. Two forms: a wait
            // variant (the serial WAP + every non-WAP submit) and a no-wait variant (the pipelined WAP + the
            // bidir bwd submit). cmdB/cmdF/cmdB_bwd are FD.pool-bound (A.pool under single_gpu, same family
            // as A.q2).
            ...
            // --nvofa semaphore chain: submit ONE cmd to A.q2 UNDER a_q2_mtx with optional
            ...
            double   last_lap_log_ms=0.0;      // rate-limit for the F lap-escape log line (~1/2s)
            uint64_t prev_ingest_cseq=0;       // cseq of the PREVIOUS ingest (span source)
            int live_n_f=cfg.fg_factor;        // F-owned live N (auto degrades from the cap)
            // Live-N feasibility control. Falling-behind is read from span>1 persistence (F actually skipping
            // sources); feasibility gets a dead band (degrade >0.90*Tsrc, upgrade needs <=0.65*Tsrc sustained)
            // + a post-change dwell. Stable N beats a higher-but-oscillating N (the mode change itself reads
            // as periodic judder).
            int up_streak=0, deg_streak=0, dwell_sets=0;
            double span_ema=0.0;
            double t_fuse_ema=0.0, t_warp_ema=0.0;   // measured set-build stage times (ms)
            // frame-holon fit state (F-local). gme_fit_ema = the CPU fit cost EMA (ms); gme_fits = the fit
            ...
            // pressure-throttle state (F-local). t_pair_ema = the full-F-pair-iteration EMA (ms) — measured
            // inline over [WAP block start … both fits done], the whole per-pair cost the deficit lives in.
            // bwd_skipping = the hysteresis latch: enter the skip regime when t_pair_ema > 0.95*pair_budget,
            // leave it when < 0.80*pair_budget (the dead-band — a single threshold flaps; two with a gap
            // between them latch). pair_budget = the arrival interval = src_interval_us/1000 (= 1000/arr_rate),
            // P's source cadence.
            double t_pair_ema=0.0; bool bwd_skipping=false;
            double t_flow_ema=0.0;          // (--fsub): EMA of the fwd fence-wait (the 1080 Ti GPU leg)
            // the pressure-TIER state (F-local). pressure_tier = the current escalation level (0..3),
            // latched with hysteresis off the SAME t_pair_ema vs pair_budget signal the bwd-skip latch reads
            // (no new EMA, no new clock). holon_pair_ctr = the monotone per-CONSUMED-pair counter that gates
            // "every Nth pair" for the object/memory work (tier 2 → period 2, tier 3 → period 4). Both
            // advance only on pairs F actually ingests (the WAP block), so the period is measured in real
            // consumed pairs. All zero at warm-up → tier 0 → no escalation.
            int pressure_tier=0; uint64_t holon_pair_ctr=0;
            uint64_t tier4_dwell=0; const uint64_t kTier4DwellPairs=90;   // shed min-hold (~0.85s @107 cons/s)
            // tier5 dwell + gov_floor live on the PRESENT thread (it owns the util read + the dwell
            // hysteresis). F READS the published g_gov_floor and applies it as max(cpu_ladder, floor) AFTER
            // the tier ladder (decoupled from bwd_skipping). One decode owner.
            // the flow-input downscale. When flow_div>1, blit Bframe[a]/Bframe[b] (full-res, RO) into
            // Bflow[0]/Bflow[1] (WW_flow×WH_flow) with a LINEAR filter, then return the Bflow views to feed
            // record_optical_flow — so the level-0 match input matches the smaller MV grid (the matcher reads
            // a_view/b_view at level 0 via UV normalized to frame_w_=WW_flow; a full-res input would make each
            // tile's block-centre offsets misalign with the input texel grid). When flow_div==1 this lambda
            // is NEVER called (the call sites pass Bframe directly). The blit runs on the SAME cmd buffer as
            // the record (cmdF for fwd, cmdB_bwd for bwd), so it is ordered before the match by the
            // RO→TRANSFER_DST→RO barriers; one ~half-res LINEAR blit per pair.
            auto flow_downsample=[&](VkCommandBuffer cmd,int a_slot,int b_slot,VkImageView& out_a,VkImageView& out_b){
                auto blit_one=[&](Img& src,Img& dst){
                    ...
                    // src (a Bframe) back to RO — the match reads Bflow (the dst), not Bframe; the Bframe RO
                    // restore keeps its layout contract clean for the next pair's upload. dst (Bflow) → RO so
                    // the OFP samples it as a_view/b_view.
            ...
            // ── forward-pass pipelining state (F-local; LIVE only under cfg.fwd_pipeline && use_wap). ──
            ...
            // g_seq is only CONSULTED when the pipeline is on. ON, g_seq LEADS f_seq by exactly one (pair N
            // recorded while pair N-1 is consumed/published) → the three live gens {N gpu, N-1 cpu, N-2 P}
            // are distinct mod NS=3.
            uint64_t g_seq=0;
            ...
            // here (the ON consume forces do_bwd off so it never reads Bframe content, which the next record
            // has already clobbered: single ofp + 2 Bframe slots cannot host a second match in flight).
            // fwd_fence is the no-wait submit's fence to block on at consume.
            using FwdPend = pfg::flow::FwdPend;   // R5 step 3c: the snapshot type lives in flow/flow_consume.hpp
            FwdPend pend{};
            // the inertia prior's CONTINUOUS per-MV-block persistence counter. F-thread-local, zero-initialised
            // ONCE here, accumulated across ALL pairs (NOT per-gen — it integrates motion HISTORY over time;
            // the per-gen hostPER bridge is only the upload vehicle). Sized mvw_f×mvh_f (== the WAP MV grid).
            // Updated right after the fwd MV+SAD copy-out is host-visible: per block static_now = (|mv| ≤ 1px)
            // && (sad_zero ≤ 8.0 sum-units) → persist = min(255, persist+16); else persist = 0 (INSTANT reset
            // on confident motion — the asymmetry by construction). Then memcpy'd into hostPER[f_gen] for A to
            // upload. Allocated only when use_inertia. std::vector = a runtime-sized zero-init heap array (no
            // per-frame alloc).
            std::vector<uint8_t> persist;
            if(use_inertia) persist.assign((size_t)mvw_f*(size_t)mvh_f, (uint8_t)0);
            // the objdump instrument — tiny per-pair BMP grids of the F data plane (post-repair dissidence
            // mask / |MV|·16 / persist) for inspecting the fields directly. Scratch sized once; output to
            ...
            // ── the object-holon — F-thread-local state + FIXED scratch (sized ONCE here, never per-pair:
            // no heap allocation inside the per-pair path). The scratch is SHARED between the fwd and bwd
            // repairs because they run sequentially within one pair (the bwd repair starts only after the
            // fwd one finished reading/writing the scratch). The two identity TABLES are separate (fwd
            // clusters ≠ bwd clusters) and persist across pairs. All sized to the full mvw_f×mvh_f grid the
            // WAP MV field uses. Allocated only when use_objects.
            pfg::flow::HolonScratch hs; hs.obj_nblk=(size_t)mvw_f*(size_t)mvh_f;   // R5 step 3a: the holons' scratch (flow/holons.hpp); the names below are aliases into it
            const size_t obj_nblk=hs.obj_nblk;
            auto& obj_label=hs.obj_label;       // per-block cluster label (-1 = not object / unvisited)
            auto& obj_bfs=hs.obj_bfs;         // BFS frontier queue (block linear indices), cap = nblk
            ...
            using ObjCluster = pfg::flow::ObjCluster;   // R5 step 3a: the type lives in flow/holons.hpp
            auto& obj_clusters=hs.obj_clusters;  // one entry per discovered component this pair
            auto& obj_used=hs.obj_used;      // top-K selection scratch (1 byte/cluster); sized to nblk (the worst-case component count) ONCE
            // Per-row scanline fill bounds within a cluster's bbox (min/max member column per grid row).
            auto& obj_rowmin=hs.obj_rowmin; auto& obj_rowmax=hs.obj_rowmax;   // sized mvh_f; reset per cluster over its bbox rows
            // the contour shape-field scratch — two fixed arrays, each sized to the full mvw_f×mvh_f grid
            // ONCE here (no per-pair heap). They are bbox-scoped per cluster (only the cluster's bbox window
            // is touched/initialized each pair, so no full-grid clear is needed), and SHARED across fwd/bwd +
            // across clusters because each cluster's chamfer fully overwrites its own bbox window before the
            // fill reads it. obj_chamf = the CHAMFER DISTANCE transform (integer 3/4 chamfer to the rim;
            // INT32_MAX = unreached/non-silhouette). obj_feat = the FEATURE transform: the linear block index
            // of the NEAREST RIM block (the rim MV is read from the field hmv[obj_feat·2] at write time).
            auto& obj_chamf=hs.obj_chamf;       // chamfer distance to the contour (rim) per silhouette block
            auto& obj_feat=hs.obj_feat;        // nearest-rim block linear index per silhouette block (feature transform)
            ...
            using ObjSlot = pfg::flow::ObjSlot;   // R5 step 3a: flow/holons.hpp
            auto& obj_slots_fwd=hs.obj_slots_fwd; auto& obj_slots_bwd=hs.obj_slots_bwd;
            // ── the scene-holon's persistent silhouette PRIOR + advection scratch. The prior is the holon's
            // BELIEF: a byte array in CUR-anchored space (the presented scene's space), in the SAME 16·r
            // quantization the dissidence masks use, PERSISTING across pairs (NOT cleared per pair). mem_adv
            // is the per-pair advected copy (prev-anchored prior pushed into THIS pair's cur space by the fwd
            // MV field). Both sized ONCE here to the full mvw_f×mvh_f grid (zero per-pair heap). Allocated
            // only when use_memory. mem_prior starts all-zero (no belief before the first confirmed pair
            // builds it).
            auto& mem_prior=hs.mem_prior;   // persistent CUR-anchored silhouette belief (16·r bytes; survives pairs)
            auto& mem_adv=hs.mem_adv;     // per-pair advected prior (cur-anchored = THIS pair's prev-prior pushed fwd)
            // ── the wake-evaporation match scratch — "the object takes its silhouette with it". mem_refresh
            // runs in CUR-anchored space; the BWD repair is the LAST writer of obj_label, so at refresh time
            // obj_label holds THIS pair's BWD (cur-anchored) cluster labels — exactly the geometry
            // mem_prior/mem_adv live in. To expire the trailing wake we need, per ARMED bwd cluster: the slot
            // MV (sweep direction), the cluster id (= the label value in obj_label, which is keep_idx[k]),
            // and the bbox. We record THAT here (a fixed [kObjSlots] table, ZERO per-pair heap) during the
            // BWD object_repair pass only; the per-row fill bounds are RE-DERIVED from obj_label inside
            // mem_refresh. Fwd repair passes wake_out=null → records nothing.
            using WakeRec = pfg::flow::WakeRec;   // R5 step 3a: flow/holons.hpp
            auto& wake_rec=hs.wake_rec;    // ARMED bwd clusters this pair (cap kObjSlots; fixed, sized once)
            if(use_objects){
                ...
                obj_chamf.assign(obj_nblk,INT32_MAX);      // chamfer distance scratch (full grid, sized once)
                obj_feat.assign(obj_nblk,0u);              // nearest-rim feature scratch (full grid, sized once)
            ...
            if(use_memory){
                mem_prior.assign(obj_nblk,(uint8_t)0);     // silhouette prior (full grid, sized once; zero belief at start)
                mem_adv.assign(obj_nblk,(uint8_t)0);       // advection scratch (full grid, sized once)
                wake_rec.assign((size_t)kObjSlots, WakeRec{-1,0,0,0,0,0.0,0.0});   // armed-bwd-cluster table (fixed, sized once)
            }
            ...
            // ── the repair routine. Runs the full object-holon for ONE anchor: connected-component cluster
            // the dissidence mask (dis byte > matte_thresh·255 — the EXACT shader OBJECT test the matte
            // uses), keep the kObjSlots heaviest clusters with mass ≥ kObjMinMass, match them to the
            ...
            //   adv_sign  : the identity-advection sign. The FWD field maps prev→cur (mv ≈ +v for an object
            //               moving +v) → +1. The BWD field is recorded with SWAPPED views (cur→prev — see the
            //               do_bwd record: mv ≈ −v) → its centroids still move +v across pairs, so the
            //               advection must NEGATE the slot mv → −1. Only the table prediction uses the sign;
            //               the inherited mv written to the field stays in the anchor's own convention (the
            //               cluster mean IS that convention).
            // wake_out/wake_n_out — when non-null (the BWD pass only), the repair records each ARMED kept
            // cluster's {cid, bbox, owner-slot mv} into the fixed wake_out[kObjSlots] table for mem_refresh's
            // wake evaporation. null (the FWD pass) → records nothing.
            //   persist_mut: "membership beats inertia" — non-null ONLY on the fwd call (persist[] is
            //                screen/prev-anchored) when use_inertia && cfg.persist_reset. Inside an ARMED
            //                cluster's filled silhouette every visited block's persistence is RESET: a block
            //                inside a tracked moving object is NOT a HUD — its static look is the FLAT-INTERIOR
            //                ILLUSION (immeasurable MV on flat content: |mv|≈0 AND sad_zero≈0, indistinguishable
            //                from a HUD by the persistence counter). The reset (a) unblocks the inheritance
            //                below and (b) releases the shader's inertia gates next pair (hostPER uploads
            //                persist[]). Real HUDs never sit inside an ARMED cluster (a mover passing over
            //                resets them transiently — they re-latch in ~8 pairs).
            auto object_repair=[&](void* mv_field, uint8_t* dis_mask, const float model6[6],
                ...
                pfg::flow::object_repair(hs, cfg, mvw_f, mvh_f, mv_field, dis_mask, model6, slots, persist_p, persist_mut, span_local, adv_sign, out_live, out_rep, out_infill, wake_out, wake_n_out);   // R5 step 3a: the body moved to flow/holons.cpp (verbatim); this wrapper keeps the call sites
            };
            // ── the scene-holon's three operations — ADVECT, MERGE, REFRESH. All pure CPU, single-threaded
            // F, zero per-pair heap (mem_prior/mem_adv are the fixed scratch above). They run only when
            // use_memory, gated at the call sites. The half-float MV decode they need is the SAME
            // half_to_float the repair uses.
            //
            ...
            // the fwd merge can still read it pre-advection (the prev-anchored merge).
            auto mem_advect=[&](const void* fwd_field){
                pfg::flow::mem_advect(hs, mvw_f, mvh_f, fwd_field);   // R5 step 3a: the body moved to flow/holons.cpp (verbatim); this wrapper keeps the call sites
            };
            // (B) MERGE — fold a silhouette prior into ONE anchor's fresh dissidence mask BEFORE clustering,
            // applying the EXPIRATION rule per block first. prior_src is mem_prior for the fwd (prev-anchored)
            // merge and mem_adv for the bwd (cur-anchored) merge (the fwd mask lives in prev space = the
            // prior's own un-advected space; the bwd mask lives in cur space = the advected prior's space).
            // The anchor's OWN field supplies the |mv| the expiration test reads (hostMV for fwd, hostMVB for
            // bwd). Per block:
            ...
            //   decay(p) = (uint8)(p·kPriorDecay) — the unconditional per-pair floor (unconfirmed → ~3 pairs).
            ...
            auto mem_merge=[&](uint8_t* dis_mask, const void* anchor_field, const uint8_t* prior_src){
                pfg::flow::mem_merge(cfg, mvw_f, mvh_f, dis_mask, anchor_field, prior_src);   // R5 step 3a: the body moved to flow/holons.cpp (verbatim); this wrapper keeps the call sites
            };
            ...
            // confirming it.
            // wake_recs/wake_count carry THIS pair's ARMED BWD clusters (recorded in the bwd object_repair).
            // After the normal max() refresh, evaporate the TRAILING wake: "the object takes its silhouette
            // with it". obj_label still holds the BWD labels (the bwd repair ran last), and the records are
            // cur-anchored = mem_prior's space. Runs only when bwd_ran (the labels are valid + the dis_b_post
            // space is the confirmed cur mask); on a bwd skip mem_refresh pure-decays and skips evaporation
            // entirely (pure decay still bounds the wake).
            auto mem_refresh=[&](const uint8_t* dis_b_post, bool bwd_ran,
                                 const WakeRec* wake_recs, int wake_count){
                pfg::flow::mem_refresh(hs, mvw_f, mvh_f, dis_b_post, bwd_ran, wake_recs, wake_count);   // R5 step 3a: the body moved to flow/holons.cpp (verbatim); this wrapper keeps the call sites
            };
            // ── the WAP per-pair CPU tail + F→P publish, factored out so the SERIAL path and the forward-
            // PIPELINED path run the SAME code (no second copy = no silent-divergence hazard). It WAITS the
            // pair's fwd fence (pc.fwd_fence), runs the holon/mem/object CPU work over the pair's DURABLE
            // per-gen host buffers (hostX[pc.f_gen]), then publishes. The parameterization is (a) the per-pair
            // values come from `pc` (so the pipeline can defer it one pair) and (b) `allow_bwd` gates the bwd
            // pass. SERIAL calls it inline with allow_bwd=true. PIPELINED calls it deferred with
            // allow_bwd=false: the bwd shares the single `ofp` + the 2 `Bframe` slots with the fwd match,
            // which the NEXT pair's in-flight fwd record has already reused, so a deferred bwd would read
            // clobbered Bframe content — the pipeline therefore runs bwd-OFF. It does NOT toggle cur_f /
            // have_prev_f / prev_ingest_cseq (those are loop-progression state the caller advances per RECORD
            // so the next match picks the right Bframe slot) nor collect the q2 copy-outs (WAP never uses
            // them). TORN-READ DISCIPLINE: pc.f_gen is the gen of the pair being consumed; in the pipeline
            // g_seq leads f_seq by one so pc.f_gen ≠ the in-flight pair's gen → this only ever reads
            // hostX[pc.f_gen] (durable post-pc.fwd_fence), never the GPU's live gen; the process-wide state
            // (persist / mem_* / obj_*) is mutated here in strict pair order.
            // R5 step 3c: the tail's state, bound by reference (run_flow stays the owner — flow/flow_consume.hpp).
            pfg::flow::ConsumeState cs{
                .hs = hs,
                .persist = persist,
                .gme_fit_ema = gme_fit_ema,
                .gme_fits = gme_fits,
                .gme_sub2 = gme_sub2,
                .gme_fit_printed = gme_fit_printed,
                .gme_vfy_dis = gme_vfy_dis,
                .gme_vfy_n = gme_vfy_n,
                .obj_cost_ema = obj_cost_ema,
                .obj_pairs = obj_pairs,
                .obj_settle_printed = obj_settle_printed,
                .t_pair_ema = t_pair_ema,
                .bwd_skipping = bwd_skipping,
                .t_flow_ema = t_flow_ema,
                .t_fuse_ema = t_fuse_ema,
                .t_warp_ema = t_warp_ema,
                .span_ema = span_ema,
                .pressure_tier = pressure_tier,
                .holon_pair_ctr = holon_pair_ctr,
                .tier4_dwell = tier4_dwell,
                .live_n_f = live_n_f,
                .mv_audit_left = mv_audit_left,
                .up_streak = up_streak,
                ...
                .flow_submit_nowait = flow_submit_nowait,
                .flow_submit_q2_chain = flow_submit_q2_chain,
                .flow_downsample = flow_downsample,
                .mv_audit_stat = mv_audit_stat,
                .objdump_grid = objdump_grid,
            };
            auto consume_wap = [&](const FwdPend& pc, bool allow_bwd){
                pfg::flow::consume_wap(ctx, cs, pc, allow_bwd);   // R5 step 3c: the tail moved to flow/flow_consume.cpp (verbatim); this wrapper keeps the two call sites
            };
            while(!g_quit&&!g_quit_threads.load()){
                ...
                // in-order ingest. F consumes last+1 (span 1) while the backlog fits the ring's safe read
                // window — it drains because F is faster on average — and jumps to newest only on a real
                // stall. Jumping straight to the NEWEST capture would skip sources → span-2 pairs → 2×
                // displacement → the walking tremble.
                //
                // TORN-READ SAFETY: `if(newest_c==last_c_seen) continue;` plus c_seq monotonicity make
                // g = newest_c-last_c_seen >= 1 here; the in-order branch's guard caps the backlog at
                // g <= cfg.ingest_backlog. The read slot (cur_c-1)%cap_slots and C's write head
                // newest_c%cap_slots are then at most ingest_backlog apart (mod cap_slots), and the invariant
                // cap_slots >= kIngestBacklog+1 (static_asserted at decl via kCapSlotsMin) guarantees a
                // >=1-slot non-collision margin → they do NOT alias; g > backlog already falls to `newest_c`
                // (a slot != C's write head). The selector is torn-read-safe by the LIVE constants.
                // --ingest-backlog: the drain cap is the runtime cfg.ingest_backlog (default kIngestBacklog=3).
                // It only ever REDUCES the compile-time kIngestBacklog, so the invariant cap_slots >=
                // kIngestBacklog+1 still bounds it (cfg.ingest_backlog <= kIngestBacklog <= cap_slots-1). Lower
                // = F jumps to newest sooner = fresher reals (less freshage) at the cost of more span-2 pairs —
                // the input-lag-floor knob (freshage is ~this × T_src).
                ...
                // record+ring-guard spin, BEFORE the fsub tf0/fence-wait).
                ...
                // consumed reads (torn-read guard). In serial both are equal → f_gen == `f_seq.load()%NS`.
                const bool fwd_pipe = (use_wap && cfg.fwd_pipeline);
                const int f_gen=(int)((fwd_pipe?g_seq:f_seq.load())%(uint64_t)NS);
                // F must not build into the generation P is still presenting. Condition: P is holding the
                // f_seq slot that F is about to overwrite (p_presenting == f_seq+1-kGenRing). With kGenRing=3
                // this only fires when span≥3 (large stalls); the ring width covers the normal case.
                //
                // THE BOUNDED ESCAPE. A naive guard spins while p_presenting==fs-2 with no exit but g_quit;
                // under a sustained deficit P can FREEZE on the oldest generation and re-store the SAME
                // p_presenting every tick, so F would spin forever, f_seq never advances → P never advances →
                // a circular liveness deadlock. The escape is bounded: spin at most ~kRingGuardSpinMax ms
                // (re-reading p_presenting each Sleep so a NORMAL transient still resolves by the wait). If P
                // is still pinned after the bound it is FROZEN, not transiently busy → stop waiting and
                // proceed. This is SAFE because P's own read-side gates (real_valid, prev_slot_safe) already
                // freeze-at-cur if F overwrites a generation P holds — the guard is a quality optimization
                // (avoid one freeze tick), never a hard correctness barrier. F advancing un-pins P (a fresh
                // f_seq) and breaks the cycle. Counted as a lap-escape.
                // the guard protects the gen F is ABOUT TO OVERWRITE. SERIAL: that pair publishes as f_seq →
                // guard vs f_seq. PIPELINED: it publishes as g_seq (g_seq leads f_seq by one) → guard vs g_seq
                // so the "kGenRing-deep" safety window stays anchored to the slot actually being clobbered,
                // not to the lagged publish counter. p_presenting holds an f_seq VALUE; with g_seq=f_seq+1 the
                // ON target fs-1 == the oldest of the 3 live gens P can hold.
                ...
                // the fwd-record cmd buffer. SERIAL (and every non-pipelined path) → cmdB/fB. PIPELINED WAP →
                // the ping-pong cmdB_fwd[g_seq&1]/fB_fwd[g_seq&1] so pair N's fwd submit (no-wait) does not
                // reset the buffer/fence pair N-1's deferred consume is still waiting on. cmdF==cmdB for
                // everything except the opt-in WAP pipeline.
                VkCommandBuffer cmdF = fwd_pipe ? cmdB_fwd[(int)(g_seq&1u)] : cmdB;
                VkFence         fF   = fwd_pipe ? fB_fwd[(int)(g_seq&1u)]   : fB;
                // --fwd-prestage: split the hRP_b[s]->hRP_b_dev[s] device copy OUT of the blocking cmdF flow
                // submit. do_prestage is live ONLY on the serial WAP + iGPU-convert path (use_fwd_prestage
                // already gates use_wap+use_igpu_convert at config time; !fwd_pipe excludes the pipeline, which
                // already overlaps this copy via its own deferred-consume). When live: record JUST the copy
                // into the ping-pong cmdF_pre[g_seq&1] and submit it NO-WAIT on the F flow lane (A.q2, under
                // a_q2_mtx via flow_submit_nowait) BEFORE cmdF — same queue, so the copy is queued ahead and
                // runs while the flow record + ring-guard spin proceed on the CPU. cmdF then SKIPS the inline
                // copy but KEEPS the hRP_b_dev[s] TRANSFER_WRITE->SHADER_READ barrier below (the memory
                // dependency edge for the unpack's read; same-queue submission order gives the execution edge).
                // OFF (do_prestage false) → the copy records inline in cmdF, byte-identical.
                ...
                if(span>1) stat_skips.fetch_add(1);   // content skip (tremble signature)
                if(use_wap){
                    // WAP mode — F ships the RECIPE (MV+SAD), not the warps. After the match it copies B's
                    // MV+SAD images into the per-gen host bridges; the presenter (G) re-warps per tick at the
                    // exact phase. No interp copy-outs. N_use still travels (phase-grid semantics in stats
                    // only). The k=0.5 warp into Cinterp is a throwaway needed to PRODUCE the MV+SAD; its
                    // Cinterp output is discarded (WAP never reads hostI).
                    // mvw/mvh MUST track the OFP grid: this local sizes the copy-out full_bic(mvw,mvh) from
                    // ofp.motion_image()/sad_field_image()/cand_image() into the host bridges AND feeds the
                    // whole CPU tail (gme_fit_affine/object_repair/matte_mass_count). At flow_div>1 the source
                    // images shrink to ofp.motion_width()×height(); sizing at (WW+7)/8 would over-read the
                    // smaller source → validation error / garbage. mvw_f/mvh_f (= ofp.motion_width()/height())
                    // IS that grid. At flow_div==1 it equals (WW+7)/8.
                    const uint32_t mvw=mvw_f, mvh=mvh_f;
                    // tp0 anchors the FULL pair-iteration time (record → copy-out → fwd submit_wait → fwd fit
                    // → bwd → bwd fit). t_pair_ema is built from it at the iteration end and drives the
                    // bwd-skip decision (read just after the fwd wait).
                    const double tp0=now_ms();
                    if(have_prev_f){
                        // at flow_div>1 downsample the pair into Bflow and feed THOSE views (matches the
                        // smaller MV grid); at flow_div==1 feed Bframe directly.
                        VkImageView fa=Bframe[prv_f].view, fb=Bframe[cur_f].view;
                        if(use_nvofa){
                            // --nvofa: the HW OFA flow provider REPLACES the classical match. cmdF so far holds
                            // ONLY the capture upload (Bframe[cur_f]) — flush it (so cur_f is on the GPU for the
                            // OFA blit), run the semaphore-chained prep→OFA→convert (it writes ofp.motion/
                            // sad_field in place, leaving them SHADER_READ_ONLY_OPTIMAL = the post-record
                            // contract), then RE-BEGIN cmdF for the gme + copy-out TAIL below (which records into
                            // cmdF as the classical path does). nvofa is gated to flow_div==1, so fa/fb = Bframe
                            // at WW; the provider downscales WW→in_w internally (mv_scale=WW/in_w). The A.q2
                            // submits (prep/convert) hold a_q2_mtx for the SUBMIT only; the F-thread blocks ONCE
                            // on fConv inside nvofa_run.
                            (void)fa;(void)fb;
                        ...
                        // ── gme-gpu: record the FWD affine fit on B, in cmdF, right after the match (MV/SAD
                        // are RO here) and BEFORE the MV/SAD copy-out (gme_record restores RO at the end, so
                        // the copy-out barriers below are valid). It writes the dis-mask into hostDIS (B import)
                        // + the 6-float model into hostGmeM; F reads both in consume_wap. step=1 (full grid —
                        // the GPU reduce is cheap; no sub2 needed). use_gate = cfg.change_gate. Only when
                        // use_gme_gpu && have_prev_f (a real fit).
                        if(use_gme_gpu && have_prev_f){
                            // --load-governor: tier-5 runs the cheapest single-pass GPU gme-fit (1 IRLS iter)
                            // to shave the per-pair cost under combat. pressure_tier here is the PREVIOUS
                            // consumed pair's level (set by the last consume_wap) — a 1-pair lag that is
                            // immaterial given the tier is dwell-held. Reachable only at tier≥5 (= only under
                            // --load-governor); equals cfg.gme_irls2?2:3 when the flag is off.
                        ...
                        // the second-best candidate copy-out — ONLY in the FWD pass (cand_image() is written at
                        // the finest level of THIS record_optical_flow; the bwd record below reuses ofp's images
                        // and would overwrite it, but this copy + the fwd-fence wait land it in hC2_b ==
                        // hostC2[f_gen] before the bwd match runs). Same RO→TRANSFER_SRC→RO barrier discipline
                        // as MV/SAD; cand_image() is left SHADER_READ_ONLY_OPTIMAL post-call. Gated on use_ambig
                        // (cand_image() is VK_NULL_HANDLE otherwise — the copy must not be recorded).
                        if(use_ambig){
                    ...
                    if(!fwd_pipe){
                        ...
                        // already signalled by submit_wait, so consume_wap's wait is a no-op (record →
                        // submit_wait → CPU tail → publish, in order, allow_bwd=true).
                        ...
                        // ofp/Bframe (cur's record already reused them).
                        flow_submit_nowait(cmdF,fF);   // route the WAP-pipelined no-wait flow submit to A.q2 under single_gpu
                        cur_pc.fwd_fence=fF;
                    ...
                    if(fg_on_prim_f&&pfg_enabled){
                        vkEndCommandBuffer(cmdB); flow_submit_wait(cmdB,fB);   // route to A.q2 under single_gpu (this branch is inactive under single_gpu — pfg_enabled false)
                        ...
                        // enqueue the slow x4 leg (sbI[k]→hI_b[gen][k]) on B.q2 WITHOUT waiting inline. The
                        // caller already waited fB (the B.q copy Cinterp→sbI[k]), so sbI[k]'s content is
                        // host-visible-ordered before this q2 submit — no semaphore needed (CPU fence-submit
                        // ordering). The fence tfb[k] is collected at SET END before the set is published to P.
                        ...
                        if(use_mv_smooth){
                            // EMA-smooth the MV field in place, then RE-warp k=0 with the smoothed field (the
                            // record_optical_flow warp above used the raw field — discarded). All interps in
                            // the set then read one smoothed MV.
                            ...
                            // pass span — the shader EMAs in VELOCITY space (mv/span) so a span-2 pair's
                            // 2x-larger raw MV doesn't corrupt the running average.
                            struct{float alpha;float cut;uint32_t w;uint32_t h;float span;}pcm{cfg.mv_smooth,kMvCut,mvw_f,mvh_f,(float)span};
                        ...
                        // k=0 copy-out → VRAM staging (sbI[0]) when overlapping; else the direct copy
                        // Cinterp→hI_b (serial fallback).
                        ...
                        const double tf0=now_ms(); vkEndCommandBuffer(cmdB); flow_submit_wait(cmdB,fB);   // route the non-WAP k=0 flow submit to A.q2 under single_gpu
                        if(cfg.latency_trace && lt_pickup_now>0.0){ const double pf=(tf0-lt_pickup_now)*1000.0;   // pre_flow = pickup→tf0 = the F non-compute window (upload+unpack+record+spin)
                            if(pf>0.0&&pf<2000000.0){ const uint64_t pv=lt_preflow_us.load(); lt_preflow_us.store(pv?(uint64_t)((double)pv*0.8+pf*0.2):(uint64_t)pf); } }
                        if(b_q2_split) q2_copy_out(0);   // launch the x4 leg on B.q2 (overlapped, no inline wait) — b_q2_split false under single_gpu
                        ...
                        for(int k=1;k<NI_use;++k){
                            ...
                            img_barrier(cmdB,Cinterp.img,VK_IMAGE_LAYOUT_GENERAL,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,VK_ACCESS_SHADER_WRITE_BIT,VK_ACCESS_TRANSFER_READ_BIT);
                            // copy-out → VRAM staging (sbI[k]) when overlapping; else direct.
                            ...
                            img_barrier(cmdB,Cinterp.img,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,VK_IMAGE_LAYOUT_GENERAL,VK_ACCESS_TRANSFER_READ_BIT,VK_ACCESS_SHADER_WRITE_BIT);
                            const double tw0=now_ms(); vkEndCommandBuffer(cmdB); flow_submit_wait(cmdB,fB);   // route the non-WAP k>0 warp submit to A.q2 under single_gpu
                            if(b_q2_split) q2_copy_out(k);   // overlap the x4 leg on B.q2 with the next warp — inactive under single_gpu
                            if(cfg.fg_auto){ const double dt=now_ms()-tw0; t_warp_ema=t_warp_ema>0.0?t_warp_ema*0.8+dt*0.2:dt; }
                            // diagnostic (dump mode only): CPU-diff this copy vs the previous one right after
                            // the fence — distinct phases MUST differ; ~0 means the warp write was lost.
                            if(cfg.dump_n>0){
                                ...
                                std::printf("[ra] dbg F warp c%llu k=%d t=%.3f rectdiff=%.3f rectsum=%llu\n",
                    ...
                    const double tf0=now_ms(); vkEndCommandBuffer(cmdB); flow_submit_wait(cmdB,fB);   // route the non-WAP no-prev/NI<1 flow submit to A.q2 under single_gpu
                    if(cfg.latency_trace && lt_pickup_now>0.0){ const double pf=(tf0-lt_pickup_now)*1000.0;   // pre_flow = pickup→tf0 = the F non-compute window
                        if(pf>0.0&&pf<2000000.0){ const uint64_t pv=lt_preflow_us.load(); lt_preflow_us.store(pv?(uint64_t)((double)pv*0.8+pf*0.2):(uint64_t)pf); } }
                    if(cfg.fg_auto){ const double dt=now_ms()-tf0; t_fuse_ema=t_fuse_ema>0.0?t_fuse_ema*0.8+dt*0.2:dt; }
                }
                // SET END — before publishing the set to P, collect ALL the overlapped q2 copy-outs
                // (tfb[0..NI_use-1]). P reads hostI right after the f_seq bump below; nothing may still be in
                ...
                // (else-if / final-else) paths use it.
                if(!use_wap){
                cur_f=prv_f; have_prev_f=true;
                prev_ingest_cseq=cur_c;   // this ingest becomes the next pair's previous
                // publish this set's pair identity + span + N BEFORE the f_seq bump that makes the set visible
                // to P (the seq_cst fetch_add/load orders them).
                ...
                // re-evaluate live N from measured capacity (auto only). Decision lags one set (this set
                // already used N_use) — applies to the NEXT set.
                if(cfg.fg_auto){
                    ...
                    if(span_ema>0.25&&live_n_f>1&&dwell_sets==0){
                        // falling-behind: F is persistently skipping sources — shed warp work.
                ...
                }   // !use_wap (shared-tail guard)
                (void)fg_on_prim_f;
            }
            // drain the last deferred pair at loop exit so its CPU tail + publish still run (the pipeline
