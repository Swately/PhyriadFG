...
void print_help(const char* a0) {
    std::printf(
        ...
        "  --gov-util-floor        A/B: OLD pure-util floor (util band alone, no FG-own-slice distress gate). DEFAULT OFF (self-keyed floor is default)\n"
        "  --gpu-priority P        GPU scheduling priority {high|realtime}: D3DKMT process class + VK global_priority (device A) +\n"
        "                          DXGI SetGPUThreadPriority(+7) (capture device); honest per-lever fallbacks. DEFAULT OFF\n"
        "  --target-output-fps F   Output cap via TRUE TICK DECIMATION (snapped to an exact divisor of refresh-hz; skipped slots do\n"
        "                          NO warp/present -> GPU cost ~ output rate). --no-decimate = the old s2 content-quantizer (A/B). 0=OFF\n"
...
void resolve_config(Config& c, bool announce) {
    ...
    // --pace-hard/--pace-vblank consumption sites are all gated on present_own_window — under the
    // default DComp overlay they are silently inert (the operator's all-flags run carried both dead).
...
bool parse_args(int argc, char** argv, Config& c) {
    for (int i=1;i<argc;++i) {
        ...
        auto parse_extra=[&](const char* arg)->int{
            ...
            if(!std::strcmp(arg,"--no-present-own-window")){ c.present_own_window=false; std::printf("[ra] --no-present-own-window: vuelve al overlay DComp (Composed: Flip, MASD ~7-13ms — el fallback de compatibilidad). DEFAULT es ON (el plano own-window clase-LSFG: HCIF + click-through + focus-restore + no-lock-out).\n"); return 0; }
            if(!std::strcmp(arg,"--no-dedup")){ c.dedup=false; std::printf("[ra] --no-dedup: los duplicados de contenido INGESTAN (pares de movimiento-cero llegan al FG); WGC conserva su cap anti-diluvio de 8ms. DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-copy-fence")){ c.copy_fence=false; std::printf("[ra] --no-copy-fence: pickup WGC por Map-retry busy-poll (el path pre-fence). DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-mv-candsel")){ c.mv_candsel=false; std::printf("[ra] --no-mv-candsel: el tile ambiguo conserva su best_mv local (sin adopción del predictor regional). DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-multicand")){ c.mc_on=false; std::printf("[ra] --no-multicand: sin selección medoid multi-candidato (el warp único directo). DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-obj-fill-rim")){ c.obj_fill_rim=false; std::printf("[ra] --no-obj-fill-rim: el repair no estampa el rim del objeto rígido. DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-phase-norm")){ c.phase_norm=false; std::printf("[ra] --no-phase-norm: fase intra-par directa del content-clock (sin el ladder normalizado). DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-cphase")){ c.cphase=false; std::printf("[ra] --no-cphase: sin el reshape de velocidad-continua en el seam de pares. DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-pace-present")){ c.pace_present=false; std::printf("[ra] --no-pace-present: el grid del output-clock sin el slew de deriva del ancla. DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-low-d")){ c.low_d=false; std::printf("[ra] --no-low-d: el ancla D phasefix completa (freshage + span entero; más input-lag, mismo freeze-floor). DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-rfp")){ c.real_fast_path=false; c.rfp_fresh=false; std::printf("[ra] --no-rfp: sin real-fast-path (limpia también --rfp-fresh); los reales siguen la cadencia D completa. DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-rfp-fresh")){ c.rfp_fresh=false; std::printf("[ra] --no-rfp-fresh: el rfp presenta el real del PAR (lagged), no el fresco — sin sawtooth, sin la ganancia de latencia. DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-asw")){ c.asw=false; std::printf("[ra] --no-asw: HOLD-at-1 en déficit (sin extrapolación forward). DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-motion-fallback")){ c.motion_fallback=false; std::printf("[ra] --no-motion-fallback: sin el fallback de movimiento-rápido a real fresco. DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-fwd-prestage")){ c.fwd_prestage=false; std::printf("[ra] --no-fwd-prestage: la copia hRP_b->dev vuelve inline al submit del flow (el gap de build completo). DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--no-fdrop")){ c.fdrop=false; std::printf("[ra] --no-fdrop: el warp se re-graba aun en ticks (pair,cand_k) byte-idénticos. DEFAULT es ON.\n"); return 0; }
            if(!std::strcmp(arg,"--dedup")){ c.dedup=true; std::printf("[ra] --dedup: descarta frames capturados DUPLICADOS de contenido (DDA compone el escritorio al refresh del monitor; el juego renderiza menos frames únicos). El FG interpola entre únicos verdaderos en vez de pares de movimiento-cero. La tasa única real (uniq=) se reporta SIEMPRE. DEFAULT ON (--no-dedup lo desactiva).\n"); return 0; }   // capture-dedup
            ...
            if(!std::strcmp(arg,"--bg-reclaim")){
                ...
                // so a bare `--bg-reclaim` before another flag keeps the shipping default 4.0). 0 disables.
                float f = 4.0f;
            ...
            if(!std::strcmp(arg,"--ts-smooth")){ if(auto v=next(arg)){ float f=(float)std::atof(v); c.ts_smooth = f<0.f?0.f:(f>1.f?1.f:f); std::printf("[ra] --ts-smooth %.2f: adaptive temporal smoothing (blend toward prev output on garbage/low-conf pixels; clean stays sharp). DEFAULT 0 (smear medido a fuente rápida 2026-07; útil en fuentes lentas).\n",c.ts_smooth); return 0; } return 1; }
            if(!std::strcmp(arg,"--multicand")){ c.mc_on=true; std::printf("[ra] --multicand: scored MULTI-CANDIDATE medoid selection (generate+discriminate+SELECT). Candidates: warp + A-only + B-only + %d speed-perturbed; medoid consensus, dispersion->crossfade fallback, Sobel edge-gate keeps the hard pick at true edges. DEFAULT OFF (alta-fuente: blur/sawtooth medidos 2026-07 — opt-in para fuentes lentas).\n",c.mc_nperturb); return 0; }
            ...
            if(!std::strcmp(arg,"--gov-util-floor")){ c.gov_util_floor=true; std::printf("[ra] --gov-util-floor: OLD pure-util governor floor (util band ALONE drives the tier floor, no FG-own-slice distress gate). DEFAULT OFF — the self-keyed floor (freshage/warp distress gates util) is the default. This is the A/B baseline: it reproduces the old tier:5-at-high-external-load false-positive. Needs --load-governor. See SATURATION_PLAN.md.\n"); return 0; }   // A/B: restore pre-0.4.0 pure-util keying
            if(!std::strcmp(arg,"--gpu-priority")){ if(auto v=next(arg)){ if(!std::strcmp(v,"high")) c.gpu_priority=1; else if(!std::strcmp(v,"realtime")) c.gpu_priority=2; else { std::printf("[ra] --gpu-priority: unknown value '%s' (use high|realtime)\n",v); return 1; } std::printf("[ra] --gpu-priority %s: the GPU scheduling-priority levers for the saturated-game regime — (1) D3DKMTSetProcessSchedulingPriorityClass(%s) at startup (the OBS 'GPU priority' mechanism; realtime typically needs elevation), (2) VK_EXT/KHR_global_priority %s on device A's queues (NOT_PERMITTED/missing-ext -> honest retry at normal), (3) IDXGIDevice::SetGPUThreadPriority(+7) on the D3D11 capture device. Each lever prints its own outcome. DEFAULT OFF. See SATURATION_PLAN.md \xC2\xA7levers-measured.\n", v, c.gpu_priority==2?"REALTIME=5":"HIGH=4", c.gpu_priority==2?"REALTIME(1024)":"HIGH(512)"); return 0; } return 1; }   // GPU scheduling-priority levers (A/B this iteration)
            ...
            if(!std::strcmp(arg,"--fps-overlay")){ c.fps_overlay=true; std::printf("[ra] --fps-overlay: live \"in->out\" fps overlay (real-captured in -> presented out) drawn TOP-LEFT on the presented frame by a compute RMW — WAP/async default path: onto wapOutA pre-blit (the --afill-safe in-place pattern; re-homed 2026-07, gap reported by chlmateus); grid sync path: onto Apresent inside bridge_present_src. DEFAULT OFF (byte-identical-off).\n"); return 0; }   // opt-in in->out fps overlay
            ...
            if(!std::strcmp(arg,"--fwd-prestage")){ c.fwd_prestage=true; std::printf("[ra] --fwd-prestage: collapse the F per-pair build gap — split the hRP_b[s]->hRP_b_dev[s] device copy out of the blocking flow submit into a double-buffered no-wait prestage on the F flow lane (A.q2), so the ~11ms non-compute B-side ingest overlaps the flow-record + ring-guard wait instead of fronting it. Shrinks freshage_ema (build ~20->~9ms target) -> lower interp latency AND D lead, no flow-quality cost. Serial WAP + iGPU-convert path only; force-off otherwise. Crash-class. DEFAULT ON (byte-identical: the copy stays inline in cmdF).\n"); return 0; }   // opt-in fwd-prestage
            ...
            if(!std::strcmp(arg,"--rfp-fresh")){ c.rfp_fresh=true; std::printf("[ra] --rfp-fresh: on a real-fast-path tick present the FRESHEST captured real ((cur_c-1)%%cap_slots) not the lagged pair real; latency measured vs c_slots[fresh].t_cap_ms (the responsiveness lever plain --rfp lacks). Content-order key KEPT at pair_c (no clock push → no reseat/freeze); the trade is a content SAWTOOTH (fresh real → stale interp), an accepted opt-in visual effect. Needs --rfp. DEFAULT OFF (alta-fuente: blur/sawtooth medidos 2026-07 — opt-in para fuentes lentas).\n"); return 0; }   // --rfp fresh-real sub-toggle
            ...
            if(!std::strcmp(arg,"--copy-fence")){ c.copy_fence=true; std::printf("[ra] --copy-fence: event-driven WGC copy-completion pickup (a fence+event wait instead of a Map-retry busy-poll, off the D3D11 context → no callback deadlock). Needs D3D11.4. DEFAULT ON.\n"); return 0; }   // copy-fence
