// PhyriadFG capture layer (STEP 4b — PURE RELOCATION from src/core/main.cpp; no logic change).
// Bodies of the CAPTURE-side D3D11/DXGI interop + iGPU convert/unpack factories declared in
// capture/capture.hpp. The find-window internals (WndFind / enum_wnd_cb) stay `static`; the main-
// called factories get external linkage. No *_spv.hpp includes (factories take the SPIR-V as a param).
...
#include "core/fg_context.hpp"   // STEP 5.2: FgContext (the C-thread's shared main()-locals as refs)
#include <cstdio>                 // std::printf (relocated C-thread body)
#include <cmath>                  // std::sqrt (igpu-field-verify CPU oracle)
#include <chrono>                 // INGEST-ASYNC: std::chrono::milliseconds (the convert-worker wait_for timeout)
...
bool d3d_init(D3D& d,int ci,bool want_dup){
    d.cap_ci=ci;   // H1: persist the chosen output index for dda_rearm()
    ...
    // STAGE-32: enable D3D11 multithread protection so callback (CopyResource) and
    // main loop (Map/Unmap) can share the immediate context safely (WGC = MSVC only).
#ifdef _MSC_VER
...
// H1 (DDA hardening): re-arm the DXGI output duplication after it dies. On Win11 (esp. 24H2) a fullscreen
// transition, a display-mode change, or an MPO plane re-plan returns DXGI_ERROR_ACCESS_LOST from
// AcquireNextFrame and INVALIDATES the IDXGIOutputDuplication; without re-arming, the capture thread would
// spin forever on the corpse and the FG freezes mid-game. We release the dead dup and re-DuplicateOutput on
// the SAME (persisted) output index, refreshing dims/format in case the mode changed. Does NOT recreate the
// D3D11 device — a genuine DEVICE_REMOVED (TDR) needs a full restart (rare; caller logs + keeps retrying).
...
bool cpipe_create(VDev& d,VkBuffer src,VkDeviceSize src_b,VkBuffer dst,VkDeviceSize dst_b,VkBuffer rgba,VkDeviceSize rgba_b,const std::vector<uint32_t>& spv,ConvPackPipe& p){
    ...
    VkPushConstantRange pcr{}; pcr.stageFlags=VK_SHADER_STAGE_COMPUTE_BIT; pcr.size=20; // {groups,is_bgra,px,is_hdr,exposure} (34-3)
...
// -- Thread C - run_capture (STEP 5.2: VERBATIM relocation of main()'s thr_c_fn lambda; no
...
void run_capture(FgContext& ctx){
    ...
    auto& c_cv = ctx.c_cv;
    // INGEST-ASYNC: the acquire-side decoupling state (touched only on the cfg.ingest_async branch below).
    auto& raw_seq = ctx.raw_seq;
    auto& dd_acq = ctx.dd_acq;
    auto& dd_uniq = ctx.dd_uniq;   // CAPTURE-DEDUP: frames ÚNICOS reales (escrito en el camino de acquire, serial + async)
    ...
    auto& dxgi_stage2 = ctx.dxgi_stage2;
            ...
            // FIX #1: pin the CAPTURE thread to a dedicated P-core (gated on cfg.pin_threads;
            // skip UINT32_MAX — unavailable id). On failure log + continue (do NOT crash).
            // S0 ablation (--pin-test, read only after cfg.pin_threads short-circuits): pin C only when
            // the active mode includes affinity (0=FULL, 1=NO-FLOW-RT, 3=AFFINITY-ONLY); 2/4 skip it.
            ...
            // S2 mode 5 = MMCSS-COMPOSITE: C gets SOFT affinity (ideal-processor hint, migratable —
            // NOT a hard pin; the affinity-trap R7 HURT B/latency) + MMCSS 'Capture'/NORMAL, with
            // elevate_thread_rt(false)=HIGHEST as the fallback when MMCSS is inactive.
            ...
            int warmup_c=0;
            double last_dd_arr_ms=0.0;   // STAGE-34-2: previous DD arrival timestamp
            // ── INGEST-ASYNC (--ingest-async, DDA-only): the ACQUIRE-ONLY loop ───────────────────────────
            ...
            // and the serial loop below runs BYTE-IDENTICAL to today.
            if(cfg.ingest_async){
                ...
                // MEDICIÓN (workflow ingest-to-refresh): desglose por-operación del lazo de acquire, para decidir
                // si el cap ~120 lo domina el map+memcpy (CPU-side → D1 lo arregla) o acquire/copy (vblank-wait /
                // 4090 saturado → nada CPU-side ayuda). EMAs locales 0.9/0.1, impresas ~1/s. Sin efecto en el pipeline.
                double _ema_acq_ms=0.0,_ema_cp_ms=0.0,_ema_mp_ms=0.0; uint64_t _dbg_n=0;
                uint64_t prev_hash=0;   // CAPTURE-DEDUP: hash del último frame PUBLICADO (persistente entre iteraciones)
                while(!g_quit&&!g_quit_threads.load()){
                    ...
                    if(FAILED(_ahr)){
                        // H1: ACCESS_LOST / mode change — re-arm exactly as the serial branch (park in 10ms backoff).
                        dd_lost.fetch_add(1);
                    ...
                    dd_present.fetch_add((uint64_t)fi.AccumulatedFrames);   // legacy counter (kept; readout uses dd_acq)
                    ...
                    // complete, so its DO_NOT_WAIT Map below almost always hits (the WGC pattern at ~262-269).
                    ...
                    {   // STAGE-34-2: arrival-delta EMA + dd_arrived (mirror the serial branch)
                    ...
                    if(have_prev){
                        ...
                        if(rk==raw_busy.load()){ stat_mapmiss.fetch_add(1); }
                            ...
                            if(mhr==S_OK){
                                ...
                                _mp_ms=now_ms()-_t_mp0;          // MEDICIÓN: Map+memcpy+Unmap (el readback CPU; D1 ataca esto)
                                // CAPTURE-DEDUP: el frame prev ya está en CPU (dst). dd_uniq cuenta únicos
                                // reales SIEMPRE (para el readout uniq=, incluso con --dedup OFF). Con
                                // cfg.dedup ON un DUPLICADO de contenido NO se publica (ni raw_seq ni notify)
                                // → el worker interpola entre únicos verdaderos en vez de un par de
                                // movimiento-cero. El descarte es LIMPIO: el Unmap ya se hizo y el slot rk
                                // queda reusable; have_prev/acq se actualizan abajo igual que en el camino normal.
                    ...
                    // MEDICIÓN (ingest-to-refresh): EMAs por-operación + impresión ~1/s (cada 120 acquires).
                    // acquire grande = esperando frames (vblank/DDA-bound, nada CPU-side ayuda);
                    // map+memcpy grande = procesando (CPU-bound → D1 lo arregla); copy+flush grande = GPU saturado.
            ...
            // MEDICIÓN serial (etiqueta "[ra-acq] (serial)"): si ves esta línea, el async NO se armó (modo serial).
            // Desglose 4-way del readback para decidir el fix corra en el modo que corra.
            double _sema_acq=0.0,_sema_cp=0.0,_sema_map=0.0,_sema_mc=0.0; uint64_t _sdbg=0;
            uint64_t prev_hash=0;   // CAPTURE-DEDUP: hash del último frame INGESTADO (persistente entre iteraciones)
            while(!g_quit&&!g_quit_threads.load()){
                const int s=(int)(c_seq.load()%(uint64_t)cap_slots);
                double lt_wgc_submit_ms=0.0, lt_wgc_compose_us=0.0;   // STAGE-112 (--latency-trace): carried from the consumed WGC slot
#ifdef _MSC_VER
                if(cfg.capture_api==CA_WGC){
                    // STAGE-121 (--copy-device): Map the ring on the SAME device the callback copied it on
                    ...
                    if(w==r) continue;
                    // STAGE-113 (--copy-fence) [CR1/CR3/CR4]: event-driven wait for the newest slot's copy. The
                    // callback signaled the fence to (w_local+1) after copying slot (w_local)%N; the newest filled
                    // slot is (w-1)%N (w_local==w-1) → its copy was signaled to value w. So wait fence>=w. We wait
                    // on the EVENT (NOT a blocking Map / NOT the context lock) so the FrameArrived callback keeps
                    // doing CopyResource/Signal concurrently (no capture freeze — CR1). Bounded 33ms (CR4): on
                    // timeout we fall straight into the existing Map(DO_NOT_WAIT)+older-slot+Sleep(1) path below.
                    // If the fence is already >= w, SetEventOnCompletion fires immediately (no spurious wait).
                    ...
                    // STAGE-38: saturated-primary capture resilience. The callback only SUBMITS
                    // the CopyResource — the 4090 executes it behind a saturated game, so the
                    // NEWEST slot's Map(DO_NOT_WAIT) often fails and C used to stall in 1ms
                    // sleeps (BR-island run: cap 30-60/s vs arr ~100 with cw fine — the tirones).
                    // Fall back to the next-older UNREAD slot: its copy was submitted a frame
                    // earlier and is almost surely complete. Costs one frame of capture age in
                    // exactly the moments the stall would have cost 1-33ms; the newest slot
                    // stays unread and is retried next cycle.
                    ...
                    // (The old code tight-spun here — the new stat exposed 3-4M iterations/s.)
...
#endif
                {
                    ...
                    double _sacq_ms=0.0;
                    {
                        ...
                        if(FAILED(_ahr)){
                            // H1: ACCESS_LOST (fullscreen transition / mode change / MPO re-plan on 24H2) or any
                    ...
                    }   // WAIT_TIMEOUT counted (capture cross-check) + H1 re-arm on ACCESS_LOST
                    ...
                    // memcpy=readback CPU puro. Decide: mapwait grande → GPU/copy (interop/DMA); memcpy grande → D1.
                    ...
                    // STAGE-34-2: DD "arrival" = a delivered frame in hand. Timestamp it and
                    // feed the arrival-delta EMA (measured at capture, before convert — closer
                    // to true source cadence than P's processed-interval, which runs away).
                    {
                        const double t_arr=now_ms();
                ...
                // STAGE-112 (--latency-trace): the two PRE-tcap (INVISIBLE-to-freshage) deltas. copy = the
                ...
                // CAPTURE-DEDUP: el frame ya está en CPU (Astage.mapped). dd_uniq cuenta únicos reales
                // SIEMPRE (para el readout uniq=, incluso con --dedup OFF; aquí solo se computa el hash +
                // el contador, sin saltar nada → pipeline byte-idéntico). Con cfg.dedup ON un DUPLICADO de
                // contenido se DESCARTA (continue: salta total_real + el convert + el c_seq.fetch_add) → el FG
                // nunca ve un par de movimiento-cero, e in= cae a la tasa única. El Unmap/ReleaseFrame ya
                // ocurrieron arriba, así que el continue es LIMPIO (sin fuga; igual que los otros del lazo).
                {
                ...
                if(!use_igpu_convert){
                    ...
                    // CRASH-FIX: when convert runs on the PRIMARY (--convert-gpu primary → use_igpu_convert
                    ...
                    const double tcv0=now_ms();   // P1 (DCAD Phase 1): time the A-convert. The iGPU branch already stores c_conv_us (4668/4677); this mirrors it so [lat-trace] conv is NON-ZERO on the single-GPU (A-convert) topology — the load-bearing FG-slice measurement. Measurement-only.
                    // CR3 (SINGLE_GPU_COLLAPSE): under single_gpu the F-thread ALSO submits flow to A.q2, so serialize
                    // C's convert submit behind a_q2_mtx (the only crash-safe lane for two threads on one VkQueue handle).
                    // The default --convert-gpu primary on a multi-GPU rig keeps flow on B.q → no race → no lock (byte-identical).
                    ...
                    // STAGE-34-3: is_hdr selects the FP16 scRGB tone-map branch (8 bytes/px src);
                    ...
                    if(cfg.igpu_field){   // STAGE-G1 (P0): 2nd G.q2 dispatch — contour field over hR_g into hFIELD_g
                    ...
                    const double tcv0=now_ms();   // STAGE-37: C's G-queue round-trip (contention probe)
                    { VkSubmitInfo si{}; si.sType=VK_STRUCTURE_TYPE_SUBMIT_INFO;
                      si.commandBufferCount=1; si.pCommandBuffers=&cmdG;
                      // STAGE-37a: submit to G.q2. When it's a real second queue C is its ONLY
                      // submitter → externally synchronized by construction, no g_q_mtx. Only the
                      // shared fallback (q2==q) still needs the lock (P also submits to that queue).
                    ...
                    { const double dt=now_ms()-tcv0;
                      const uint64_t prev=c_conv_us.load();
                      c_conv_us.store(prev?(uint64_t)((double)prev*0.8+dt*1000.0*0.2):(uint64_t)(dt*1000.0)); }
                    // STAGE-G1 (P0) --igpu-field-verify: CPU re-derive the Sobel field + compare to the GPU field (D-13 byte oracle).
                    // FIX 3 (muestreo step-16): el Sobel CPU full-frame O(W×H) (~2M px) amortizado a 1/120 dejaba un pico
                    // periódico ~20-40ms (congelación). Ahora se MUESTREA con paso 16 (~8K px a 1080p) CADA frame → coste
                    // sub-ms constante, sin pico → sin congelación. El COMPUTE corre cada frame (ya es barato); sólo el
                    // printf sigue limitado a ~1/120 (vfy_n) para no inundar el log. El conteo de diferencias es sobre
                    // los píxeles MUESTREADOS (etiquetado "muestreo step-16"). Sigue siendo un oráculo debug default-off.
...
// PROMPT publish (no STAGE-85 deferral): total_real + c_seq are bumped the instant the convert fence
// signals → the freshest converted frame reaches F/P at TODAY's age. The worker is joined in main()
// BEFORE any convert/Vulkan teardown.
void run_convert_worker(FgContext& ctx){
    ...
    while(!g_quit&&!g_quit_threads.load()){
        ...
        if(!use_igpu_convert){
            ...
            // FIX 3 (muestreo step-16): el Sobel CPU full-frame O(W×H) (~2M px) amortizado a 1/120 dejaba un pico
            // periódico ~20-40ms (congelación). Ahora se MUESTREA con paso 16 (~8K px a 1080p) CADA frame → coste
            // sub-ms constante, sin pico. El COMPUTE corre cada frame; sólo el printf sigue limitado a ~1/120 (vfy_n).
            static uint64_t vfy_n=0;
            if(cfg.igpu_field && cfg.igpu_field_verify && hostFIELD[s]){
                const uint32_t* src=(const uint32_t*)hostR[s];
                const uint32_t* gpu=(const uint32_t*)hostFIELD[s];
        ...
        total_real.fetch_add(1);        // PROMPT publish (no STAGE-85 deferral)
        c_seq.fetch_add(1);
        c_cv.notify_all();
    }
}
