...
// Thin forwarders to the hal binary16 codec (phyriad::hal,
// framework/hal/include/phyriad/hal/Simd.hpp). For the batch F16C decode-once (gme_fit), see
// ra::decode_f16 (ra_simd.cpp).
...
// ─── COM release helper ────────────────────────────────────────────────────────
// A header template (general util) so every module sees it — capture's COM objects
...
// --upload-xfer: optional concurrent_fams/n_fams (default nullptr/0 → EXCLUSIVE). When non-null the image
// is created VK_SHARING_MODE_CONCURRENT over the given families so it can be WRITTEN on A.qT (upload) and
// READ on A.q (warp) with NO queue-family ownership transfer (the no-QFOT path; mirrors hbuf_import's
// q2_shared buffer precedent). Used ONLY for the wap-input images and ONLY when xfer_on. Note: CONCURRENT
// can cost lossless compression on some HW — acceptable here (transient per-pair upload targets).
...
// Every host buffer is an _aligned_malloc + hbuf_import, so the same CPU pointer is importable on
// multiple devices — incl. G as SSBO.
...
// Device-local buffer (no host import — VRAM only).
...
// img_barrier sets src/dstQueueFamilyIndex to VK_QUEUE_FAMILY_IGNORED — the correct "no ownership
// transfer" form for BOTH sharing modes. A REAL family index (e.g. 0) would be a malformed
// queue-family ownership transfer the moment a CONCURRENT image is barriered on A.qT.
...
bool vk_wait_live(VkDevice dev,VkFence f) noexcept;   // core/globals.hpp — the bounded, device-loss-aware wait (declared here so the oneshot template sees it)
template<class R> void oneshot(VDev& d,R rec){ VkCommandBufferAllocateInfo cbai{}; cbai.sType=VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO; cbai.commandPool=d.pool; cbai.level=VK_COMMAND_BUFFER_LEVEL_PRIMARY; cbai.commandBufferCount=1; VkCommandBuffer cmd; vkAllocateCommandBuffers(d.dev,&cbai,&cmd); VkCommandBufferBeginInfo bi{}; bi.sType=VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO; bi.flags=VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT; vkBeginCommandBuffer(cmd,&bi); rec(cmd); vkEndCommandBuffer(cmd); VkSubmitInfo si{}; si.sType=VK_STRUCTURE_TYPE_SUBMIT_INFO; si.commandBufferCount=1; si.pCommandBuffers=&cmd; VkFenceCreateInfo fci{}; fci.sType=VK_STRUCTURE_TYPE_FENCE_CREATE_INFO; VkFence f; vkCreateFence(d.dev,&fci,nullptr,&f); vkQueueSubmit(d.q,1,&si,f); vk_wait_live(d.dev,f); vkDestroyFence(d.dev,f,nullptr); vkFreeCommandBuffers(d.dev,d.pool,1,&cmd); }
void submit_wait(VDev& d,VkCommandBuffer cmd,VkFence f);   // device-lost via the shared submit helpers (covers their G/B/setup callers)
// Identical to submit_wait but targets d.q2 — for non-present A-work (F's flow, C's convert) so A.q stays
// exclusive to P (present). Caller must guarantee a SINGLE submitting thread per queue handle (Vulkan
// external sync is per-handle): on device A, d.q2 is a same-family split-queue (q2mode=1) so A.pool cmd
