// PhyriadFG core/device layer: the vdev_create / vdev_destroy bodies (declared in core/device.hpp).
...
bool vdev_create(VkPhysicalDevice phys,VDev& d,bool want_swap,bool want_extmem_win32,bool prefer_same_family_q2,bool want_xfer_q,bool want_ofa){
    ...
    // (VK_KHR_external_memory_win32) synced by a keyed mutex (VK_KHR_win32_keyed_mutex). Both are
    // queried here and enabled ONLY when want_extmem_win32 (surface mode); when off the device's
    ...
    // first-hand from the device via a VkPhysicalDeviceFeatures2 *query*. This chain is LOCAL to the
    // read — it is NOT chained onto the vkCreateDevice pNext below and these features are NOT added to
    // the enabled set, so the device-creation path is unchanged. The feature structs are core promotions
    // (Vulkan 1.2 / 1.3) with the KHR aliases, queried unconditionally so an arbitrary-vendor GPU is
    // correctly NAMED; the A/B/G role-routing ignores these fields.
    {
    ...
    // Enable the win32 external-memory + keyed-mutex pair for the A-side bridge. The win32 import
    // needs VK_KHR_external_memory (the base, core in 1.1) + its win32 child; the keyed mutex is the
    // cross-device sync chained on the blit submit.
    ...
    // Pick a second queue (used by G to split convert off the present queue; created on A/B too but
    // unused there). Prefer (a) a DISTINCT compute-only family (AMD async compute), else (b) a second
    // queue in qfam (queueCount>1), else (c) shared fallback (q2=q).
    uint32_t qfam2=d.qfam; int q2mode=2;   // 0=split-family, 1=split-queue, 2=shared
    // A.q2 lock-free partition: when prefer_same_family_q2 is set (device A only) the split-FAMILY
    // search is SKIPPED so the mode falls through to the SAME-family split-queue (q2mode=1 when
    // queueCount>1). A.pool cmd buffers can then submit to A.q2 with NO family trap / NO ownership
    ...
    d.qfam2=qfam2;
    // --upload-xfer: find a TRANSFER queue family DISTINCT from qfam/qfam2 — prefer a COMPUTE&&!GRAPHICS
    // family (a true parallel engine that can ALSO run the in-upload median COMPUTE dispatch), else a
    // transfer-only family. UINT32_MAX → no transfer engine → the feature force-disables downstream (the
    // upload stays on the graphics queue).
    if(want_xfer_q){
    ...
    // --nvofa: pick the OPTICAL-FLOW queue family (its OWN family on the 4090). Gated on want_ofa AND the
    // extension being exposed; if no OFA family exists the feature force-disables (the ofaQueue stays null
    // → the runtime falls back to the classical OFP). The OFA family is distinct from qfam/qfam2/qfamT
    // (TRANSFER+OPTICAL_FLOW only) — it carries no graphics/compute, so uploads + the convert dispatch +
    // layout transitions all stay on qfam (q); ofaQueue runs only vkCmdOpticalFlowExecuteNV.
    ...
    // Append the transfer-queue family at the index-relative next slot (never a hardcoded index).
    ...
    // --nvofa: append the OPTICAL-FLOW queue family (index-relative next slot). Only when an OFA family was
    // found AND it is DISTINCT from every family already requested (the distinct-guard keeps a
    // duplicate-family create-info — illegal — out if a GPU folds OFA into an existing family). If it
    // coincides, ofaQueue reuses that family's queue 0 below.
    ...
    if(use_ofa){ exts.push_back(VK_NV_OPTICAL_FLOW_EXTENSION_NAME); exts.push_back(VK_KHR_FORMAT_FEATURE_FLAGS_2_EXTENSION_NAME); exts.push_back(VK_KHR_SYNCHRONIZATION_2_EXTENSION_NAME); }   // VK_NV_optical_flow REQUIRES both VK_KHR_format_feature_flags2 AND VK_KHR_synchronization2
    // The nvofa_convert pass binds the OFA flow/cost as rg16i + r8ui STORAGE images (mutable views).
    // R16G16_SINT and R8_UINT carry the STORAGE_IMAGE format feature only when
    // shaderStorageImageExtendedFormats is enabled — without it the storage-image accesses are invalid
    // (VUID-vkCmdDraw-None-09600). Enable it only on the OFA path; dci.pEnabledFeatures stays null
    // otherwise. enFeat must outlive the vkCreateDevice call (this scope).
    ...
    // --upload-xfer: the transfer queue + its pool + the two TIMELINE semaphores. Gated on a real qfamT;
    // enumerates ALL families + logs the pick.
    if(want_xfer_q){
        ...
        if(d.qfamT!=UINT32_MAX){
            ...
            std::printf("[ra] --upload-xfer: NO transfer/async-compute family distinct from qfam(%u)/qfam2(%u) on '%s' — upload stays on the graphics queue (feature off)\n", d.qfam, d.qfam2, d.name);
        }
    }
    // --nvofa: the OFA queue + its command pool + the device OF PFNs. Gated on a real OFA family; the queue
    // is from queue 0 of ofaQfam. The pool is family-bound to ofaQfam so a cmd buffer recorded from it
    // submits to ofaQueue with no family trap. If ANY PFN fails to load (a driver that exposes the ext but
    // not the entry points) the feature force-disables (ofaQueue cleared → the runtime falls back to the
    // classical OFP).
    if(use_ofa && d.ofaQfam!=UINT32_MAX){
        ...
        if(!d.pfnCreateOFSession||!d.pfnDestroyOFSession||!d.pfnBindOFImage||!d.pfnCmdOFExecute||!d.ofaQueue){
            std::printf("[ra] --nvofa: OF PFN/queue load failed on '%s' — disabling (classical OFP)\n", d.name);
