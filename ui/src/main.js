  ...
  },
  {
    ...
    controls: [
      ...
      { flag: "--no-single-track", type: "switch-off", default: true, name: "Single-track synthesis",
        desc: "DEFAULT ON (v3.1, operator-eye PASS: smooth, judged better than LSFG). Presents the pure B-track (cur warped back to phase) + crisp reals on proven-static pixels (HUD/grid). Off = the old A/B blend composite. WAP-only." },
  ...
  },
  {
    ...
    controls: [
      ...
      { flag: "--no-bg-reclaim", type: "switch-off", default: true, name: "BG reclaim",
        desc: "DEFAULT ON (strength 4.0, hard snap). TILE-scale gravity fix: a background-content tile carrying an object-like MV (the matcher straddled the silhouette) is damped to the gme model. Sibling of BG-snap (which is contour-band only). Needs gme." },
      { flag: "--bg-reclaim-strength", type: "number", default: "4.0", min: 0, max: 4, step: 0.5, name: "BG reclaim: strength",
        desc: "Damp weight scale [0,4] (1 = soft LSFG-style decay, 4 = hard snap-to-model, the shipping default). 0 = off." },
